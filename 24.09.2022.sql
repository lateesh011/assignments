...
Select member.member_id,member.member_name,count(issue.Book_No) as booksissued,member.max_books_allowed  from issue inner join member on issue.Member_Id=member.member_id  group by member.member_id,member.member_name,member.max_books_allowed having member.max_books_allowed<=count(issue.Book_No)
